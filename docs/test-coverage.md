...
- **`accounts.deactivate` / `reactivate`** (open question #8, added 2026-08-18) — closing an empty account then reopening it, with the reopen asserted by **posting money again** rather than by reading the column back, since the column flipping is the mechanism and money moving is the claim; closing a funded account returns `422 account_not_empty` **and leaves it active**, so a refusal cannot mean "changed, then complained"; a second close is idempotent rather than a `409`, because a retried request after a dropped connection must not read as a failure; another org's id is `404`, never `422`, since `account_not_empty` would itself confirm the row exists elsewhere; a viewer is `403` before the handler runs; both actions land in the audit log. The inactive-account posting test now closes through **the real procedure** — it used to `UPDATE ledger_account SET active = false` in raw SQL, and that hack's removal is the clearest evidence the gap is closed
...
- **A balance that would overflow the column is a typed refusal, not a crash** (open question #27, added 2026-08-17). Two transfers whose amounts are *each* storable — one at int8's ceiling, one of `0.01` — return `422 balance_limit_exceeded`. The point of the pair is that **a per-amount bound is not a per-balance bound**: `parseBoundedAmount` passes both, and only the accumulated balance is out of range. The second test asserts the three properties that separate a domain refusal from a crash — the balance is unchanged, the transaction was never written, and the rejection is in the audit log. Written and run *against unmodified code first*, where it produced `22003 value "9223372036854775808" is out of range for type bigint` as an unaudited 500; fixing an unreproduced bug is how a wrong deferral reason gets recorded (see #14)
...
- `accounts.e2e.ts` — a new org's accounts screen shows the **empty** state and *not* the error state. On a ledger these mean opposite things: one invites you to create an account, the other means the figures on screen may be nothing at all. Then: an admin creates an account through the real dialog, lands on its detail page, and finds it in the list
- `transfer.e2e.ts` — the money path. Two accounts created through the UI (`external` funding, `normal` wallet), a transfer through the two-stage review/confirm flow, then a reversal behind its typed confirmation. The reversal assertions are about the *pairing*, not just about a page rendering: it navigates to a **different** transaction id (history is append-only, so a correction is a new entry), the "an earlier transaction" link's `href` is asserted to be the original's exact id, and the legs are checked to be mirrored

**`support/select.ts` — why these specs stopped flaking.** The Phase 6c versions were deleted for passing on one run and failing the next, diagnosed as a popup that "stays mounted and swallows the next click". Half right. Base UI never unmounts a closed `Select` popup at all — it marks it `data-closed` and leaves it in the document — so `[data-slot="select-content"]` accumulates one match per select ever opened and is a **strict-mode violation from the second picker onward**. The helper narrows to `[data-open]`, of which there is at most one, and waits only on that state appearing and going. No wait anywhere in the suite is on a duration, and `playwright.config.ts` still sets `retries: 0` locally.

**Three defects these specs found that 763 unit and integration tests did not:**

- **`/transfer` was broken in a real browser** (2026-08-18). `packages/api/contracts/money` imported a constant from `@fintech-ledger-sandbox/db/posting`, which dragged drizzle, the Postgres driver and `node:crypto` into the console bundle. Vite warns and builds anyway; the component suite runs in happy-dom under Node where `node:crypto` resolves. See `architecture.md` → "Workspace package build contract"
- **The reverse dialog stated a falsehood about a money operation** — "reversals are not deduplicated … this will succeed" — which migration `0007` had made untrue two days earlier
- **A latent race in one of these very specs.** Creating an account navigates to its detail page, so asserting a row in the list passed only by beating the router. It surfaced the moment a second account was created in the same run, which is why the bar here is *passes repeatedly*, not *passed once*

**What e2e still does NOT cover — stated plainly, because partial coverage reported as complete is worse than none:**

- **Viewer-role behaviour.** Every spec acts as an org admin. A second member in the same org needs an invitation flow the console does not have, and seeding a `member` row directly would skip the real routing this suite exists to exercise
...
### `packages/api/src/routers/pagination.test.ts` — cursor paging on the reads that gained it (Phase 7a; `approvals.listPending` added 2026-08-17)

Closes open questions #6 and #7, and #29. `transactions.list` was already paginated and stays covered in `reads.test.ts`.
...
- **`approvals.listPending` walks the whole maker-checker queue past its old 100-row ceiling** (open question #29), through the real `submitPending` rather than hand-inserted rows — the claim being tested is that the *queue* is walkable, and a queue filled some other way is not the queue
- **The approvals walk is asserted ascending, and asserted the way that cannot flake.** Ordering here is FIFO — oldest first, so the cursor predicate is `>` rather than the audit log's `<`. The obvious test, "the walk equals submission order", is wrong: six inserts in a tight loop can share a millisecond, and order is then decided by the random-uuid `id` tiebreaker. What is asserted instead is that the composite `(createdAt, id)` pair strictly increases across the entire walk — the ordering contract itself, never trivially true, and immune to a millisecond tie
