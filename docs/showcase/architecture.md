...
> ⚠️ Honest gaps, still open: **error monitoring is not a gap** — [`docs/development/tech-stack.md`](../development/tech-stack.md) declares it `none` for this sandbox, a decision rather than a debt, and this callout used to misreport it as undelivered. What *is* open: maker-checker is enforced only in the browser, `approvals.approve` can double-post, and the database pool carries no statement timeout — [`docs/open-questions.md`](../open-questions.md) #25, #26, #28. And the one that undercuts every green check on this page: **CI has never executed a single run** (#10) — the workflow is correct, the GitHub account is billing-locked, so every check here has only ever passed on a laptop.
