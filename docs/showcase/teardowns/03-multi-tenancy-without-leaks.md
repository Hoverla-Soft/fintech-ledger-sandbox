...
- ⚠️ **No Postgres row-level security.** Isolation for *reads* is enforced in the API middleware and repository layer, not by the database. ADR 0005 states this openly: a background job or script talking to `packages/db` directly bypasses the middleware; composite FKs stop it writing cross-org rows, but nothing at the DB level stops it *reading* them. Any such caller must derive `org_id` explicitly.
- ⚠️ **The client derives its own role.** No procedure returns the caller's role (ADR 0009 logs this as open question #1), so the console imports the same `toLedgerRole` from `packages/api` and applies it to the Better Auth member row client-side. The result is an affordance hint only — it can be momentarily stale in a way the server's per-request lookup never is, and writes enforce server-side on every request.
