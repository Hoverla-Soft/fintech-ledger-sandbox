...
flowchart TD
    ...
    V --> D["Docs synced: ADRs,<br/>test-coverage.md, showcase"]
...
Honest gaps, on the record here and in the [security checklist](security.md):

- ⚠️ **CI does not gate, and is not currently running.** It has run: thirteen times, seven of which executed the full five-step suite and passed. Every run since **2026-08-16 21:03 UTC** is annotated *"The job was not started because your account is locked due to a billing issue"* and dies at zero steps — regardless of which account pushes, Dependabot included. Both the organization and the personal billing pages show a zero balance and nothing due, so this is a stuck flag rather than an unpaid invoice. The workflow file is correct and proven; nothing is executing it. Branch protection is a further step that cannot be attempted until runs resume. Note also that Railway deploys on push independently of the Actions result, so even a running CI would report rather than gate.
- ⚠️ **E2e coverage is deliberately thin: 4 specs.** Specs driving account creation and transfer were written, flaked across runs on a Base UI `Select` interaction, and were **removed** — a test that answers differently for identical code is worse than no test. Those flows stay covered by the component and API suites; the browser-level gap is recorded (item #9).
- **Maker-checker was enforced only in the browser — now closed.** `transactions.create` did not read the org's `requireTransferApproval` flag, so an admin could post straight past the approval queue by calling the API. Every direct balance change now sits on a shared `directPostProcedure` rung ([`procedures.ts`](../../packages/api/src/procedures.ts)) rather than each handler calling a guard, which is the fix that matters: the first version *was* a per-handler helper and three handlers did not call it — `reverse`, `seed`, and `reset`, all three demonstrated against a real database before being closed structurally.
...
The same pattern repeats across the doc set: [`coding-rules.md`](../development/coding-rules.md) holds code-level conventions (framework-agnostic on purpose), [`test-coverage.md`](../test-coverage.md) answers "is there a test for X" without grepping, and the [security checklist](security.md) is where anything unconfirmed gets written down instead of silently assumed — resolved rows keep their post-mortems (item #14 records that a config change deferred as "a wide diff" measured out at zero errors: "worth measuring before deferring next time").
