...
- Required Railway variables: server needs `DATABASE_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `CORS_ORIGIN` (all validated at import by `packages/env`); web needs `VITE_SERVER_URL` **at build time** — Vite inlines it into the bundle, so changing it requires a redeploy, not a restart. `PORT` is injected by Railway and read by both apps. `CORS_ORIGIN` must be the **web service's** public origin and `BETTER_AUTH_URL` the **server's own** — Hono's `cors()` matches `CORS_ORIGIN` byte-for-byte against the browser's `Origin` header, so a wrong value returns no `Access-Control-Allow-Origin` at all and every request fails CORS with no server-side error. `up.railway.app` is on the Public Suffix List, so the two services are different *sites* and session cookies are cross-site; `packages/auth` already sets `SameSite=None; Secure` for that.
