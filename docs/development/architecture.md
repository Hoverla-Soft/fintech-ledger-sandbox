...
| `apps/web` | The React + TanStack Router console (the "sandbox" UI). Routes, screens, layouts, feature components. Consumes the oRPC router **types** from `packages/api`, plus a small number of **runtime values** through that package's declared subpath exports — the currency allowlist and minor-unit exponent (`contracts/currencies`) and the money parser and its bounds (`contracts/money`). Those are re-exports of `packages/core`, routed through `packages/api` deliberately so the console never takes a direct dependency on the domain package and the one-way direction above is preserved. Added Phase 5a; see `docs/adr/0002-money-representation.md` for why the exponent must never be hardcoded in the app. |
